...
export default function TransactionForm() {
  ...
  const schema = yup.object().shape({
    ...
    value: yup
      .number().typeError('O campo deve conter um valor numérico')
      .required("O campo é obrigatório")
      .min(0.01,'O campo deve conter um valor positivo'),
    type: yup.string().required('O campo é obrigatório'),
    date: yup.date().required('O campo é obrigatório').max(new Date(), 'O campo não pode conter uma data futura'),
  });
    ...
    <>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            ...
            <Box display={'flex'} flexDirection={'row'} gap={2} sx={{justifyContent: "space-between"}} fullWidth>
              <Box>
                <Box>
              ...
              <Box fullWidth>
                ...
                <InputDate name="date" error={!!errors?.date} helperText={errors?.date ? errors?.date?.message : ''}
                           control={control} defaultValue={dayjs(transactionData?.date)}/>
              </Box>
